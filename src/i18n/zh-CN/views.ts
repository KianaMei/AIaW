export default {
  ...
  providerSetting: {
    ...
    logo: '图标',
    type: '服务商类型',
    typeCaption: '该服务商使用的 API 协议类型',
    apiHost: 'API 地址',
    apiHostCaption: 'API 请求的基础 URL（例如：https://api.example.com）',
    apiKey: 'API 密钥',
    apiKeyCaption: '用于身份验证的 API 密钥',
    enabled: '启用',
    enabledCaption: '启用或禁用此服务商',
    models: '可用模型',
    modelsCaption: '该服务商支持的模型 ID 列表。可以手动输入或从服务商获取。',
    fetchModels: '从服务商获取模型',
    fetchModelsSuccess: '成功从服务商获取 {count} 个模型',
    fetchModelsFallback: '获取模型失败（{error}）。使用 {count} 个静态模型作为备用。',
    fetchModelsStatic: '已加载该服务商的 {count} 个静态模型',
    fetchModelsError: '获取模型失败：{error}',
    validate: '验证服务商',
    validateSuccess: '服务商验证成功',
    validateError: '服务商验证失败：{error}',
    valid: '有效',
    invalid: '无效',
    updateSuccess: '服务商更新成功',
    updateError: '更新服务商失败：{error}',
    delete: '删除服务商',
    deleteConfirmTitle: '删除服务商',
    deleteConfirmMessage: '确定要删除服务商"{name}"吗？此操作无法撤销。',
